...
while True:
    ...
    for bbox in bboxes:
        ...
        # cv2.imshow("Age Gender Demo1", imgBG)
    ...
    for face_loc, name in zip(face_locations, face_names):
        ...
        try:
            if (name != 'Unknown'):
                # img = cv2.imread(f'{images_path}/{name}.png')
                # img = cv2.imread('1.png', cv2.IMREAD_UNCHANGED)
                # img = cv2.imread('Abba_Eban_0002.png', cv2.IMREAD_UNCHANGED)
                # img = cv2.imread('photos/10photos0/Abba_Eban_0002.png', cv2.IMREAD_UNCHANGED)
                img = cv2.imread(fr'photos/10photos0/{name}.png', cv2.IMREAD_UNCHANGED)
                imgBG = cvzone.overlayPNG(imgBG, img, (0, 0))
                # cv2.waitKey(500)
        except:1
    cv2.waitKey(1)
    cv2.imshow("Detection..", imgBG)
    # try:cv2.destroyWindow("Age Gender Demo1")
    # except:1
    # cv2.waitKey(0)
