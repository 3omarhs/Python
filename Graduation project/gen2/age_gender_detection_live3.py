        ...
        for bbox in bboxes:
            ...
            #     print("Gender : {}, conf = {:.3f}".format(gender, genderPreds[0].max()))
                ...
                if (name != 'Unknown'):
                    ...
                    img = cv2.imread('Abba_Eban_0002.png', cv2.IMREAD_UNCHANGED)
                    ...
                    # img = cv2.imread('1.png', cv2.IMREAD_UNCHANGED)
                    # # img = cv2.imread('Abba_Eban_0001.png', cv2.IMREAD_UNCHANGED)
                    # # res = cv2.resize(img, dsize=(250, 250), interpolation=3)# cv2.INTER_CUBIC)
                    # # img = img[::2, ::2]
                    # img.repeat(2, axis=0).repeat(2, axis=1)
                    #
                    # # imgBG[0:250, 0:250] = img       #(250,250,3)
                    # # img = cv2.imread(f'{images_path}/{name}.jpg', cv2.IMREAD_UNCHANGED)
                    # # img = cv2.imread(f'D:/Backup/PycharmProjects/datasets/10photos/Abba_Eban_0001.jpg')
                    # imgBG = cvzone.overlayPNG(imgBG, img, (0, 0))
                    # # cv2.imshow(name, img)
                ...
                cv2.imshow("Age Gender Demo", imgBG)
