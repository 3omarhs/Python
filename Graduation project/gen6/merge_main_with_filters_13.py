        ...
        if current_run == 'train':
            ...
            if category == 'Male':
                ...
                with open(f'{encoding_images_path}/{category}_names.txt', "rb") as fp:  # Unpickling
                    self.known_face_names_m = pickle.load(fp)
                    global Male_list
                    Male_list = self.known_face_names_m
            else:
                with open(f'{encoding_images_path}/{category}_encoding.txt', "rb") as fp:  # Unpickling
                    self.known_face_encodings_f = pickle.load(fp)
                with open(f'{encoding_images_path}/{category}_names.txt', "rb") as fp:  # Unpickling
                    self.known_face_names_f = pickle.load(fp)
                    global Female_list
                    Female_list = self.known_face_names_f
            print("Encoding images loaded")
        ...
        while True:
            ...
            if run_fully:
                ...
                for bbox in bboxes:
                    ...
                    try:
                        blob = cv2.dnn.blobFromImage(face, 1.0, (227, 227), MODEL_MEAN_VALUES,
                                                     swapRB=False)  # function returns a blob which is our input image after mean subtraction, normalizing, and channel swapping.
                        genderNet.setInput(blob)
                    except:
                        1
                    genderPreds = genderNet.forward()
                ...
                for face_loc, name in zip(face_locations, face_names):
                    ...
                    # filter1 = 'Angelina Jolie'
                    # try:
                    #     filter_w.show_filter(filter2, eyes, faces, screen)
                    # except:
                    #     1
                    ...
                    if gender == 'Male':
                        try:
                            # filter.show_filter(filter2, eyes, faces, frame)
                            ########################################################################
                            # filter_name = filter2
                            filter_name = 'hair1.png'
                            global FILTER_DIR
                            FILTER_DIR  = 'C:/Users/Omar Hassan/PycharmProjects/Graduation project/gen4/try together 2/data/filters/'
                            if os.path.exists(FILTER_DIR + filter_name[:-4] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-4] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-5] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-5] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-1] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-1] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-2] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-2] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-3] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-3] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-6] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-6] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-7] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-7] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-8] + ".png"):
                                image = cv2.imread(FILTER_DIR + filter_name[:-8] + ".png")
                            elif os.path.exists(FILTER_DIR + filter_name[:-9] + ".png"):
                ...
                if gender == 'Male':
                    ...
                    if keyboard.is_pressed('right'):
                        ...
                        filter3_f -= 1
                    try:
                        filter_w.show_filter(str(Female_list[filter3_f]), eyes, faces, frame)
                    except:
                        1
