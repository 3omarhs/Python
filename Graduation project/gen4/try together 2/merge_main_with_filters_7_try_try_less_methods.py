...
def main_GUI():
    ...
    ###
    frameOpencvDnn = small_frame.copy()
    ...
    frameFace, bboxes = frameOpencvDnn, bboxes
            ###
    ...
    for face_loc, name in zip(face_locations, face_names):
        ...
        if (name != 'Unknown'):
            ...
            ###
            pos = [50, 250]
            hf, wf, cf = personBG.shape
            hb, wb, cb = imgBG.shape
            *_, mask = cv2.split(personBG)
            maskBGRA = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGRA)
            maskBGR = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
            try:
                imgRGBA = cv2.bitwise_and(personBG, maskBGRA)
            except:
                imgRGBA = cv2.bitwise_and(personBG, maskBGR)
            ...
            imgBG = cv2.bitwise_and(imgBG, imgMaskFull2)
            imgBG = cv2.bitwise_or(imgBG, imgMaskFull)
            ###
            global old_img
            image = old_img
            ...
            try:
                ...
                hf, wf, cf = img2.shape
                hb, wb, cb = imgBG.shape
                *_, mask = cv2.split(img2)
                maskBGRA = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGRA)
                maskBGR = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
                try:
                    imgRGBA = cv2.bitwise_and(img2, maskBGRA)
                except:
                    imgRGBA = cv2.bitwise_and(img2, maskBGR)
                imgRGB = cv2.cvtColor(imgRGBA, cv2.COLOR_BGRA2BGR)
                imgMaskFull = np.zeros((hb, wb, cb), np.uint8)
                imgMaskFull[pos[1]:hf + pos[1], pos[0]:wf + pos[0], :] = imgRGB
                imgMaskFull2 = np.ones((hb, wb, cb), np.uint8) * 255
