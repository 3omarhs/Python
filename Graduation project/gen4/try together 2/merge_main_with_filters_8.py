...
def main_filter(mode_selected):
    ...
    if __name__ == "__main__":
        ...
        while True:
            ...
            # screen = cv2.imread(r'C:\Users\Omar Hassan\PycharmProjects\Graduation project\gen4\photos\BG.jpg')
            ...
            # face_cascade = cv2.CascadeClassifier("data/new_cascade/haarcascade_frontalface_default.xml")
            # eye_cascade = cv2.CascadeClassifier("data/new_cascades/haarcascade_eye.xml")
            face_cascade = cv2.CascadeClassifier('data/new_cascades/haarcascade_frontalface_default.xml')

            faces = face_cascade.detectMultiScale(frame, 1.3, 5)
            for (x, y, w, h) in faces:
                frame1 = cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)  #cv2.rectangle(image, start_point, end_point, color, thickness)
                # roi_gray = frame[y:y + h, x:x + w]
                # roi_color = frame[y:y + h, x:x + w]
                # eyes = eye_cascade.detectMultiScale(roi_gray)

            name = 'Ashton Kutcher (6).png'
            ...
            # try:
            frame1 = filter_1.show_filter(filter1, frame, faces, frame1)
            # except:
            #     1
